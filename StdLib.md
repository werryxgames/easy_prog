...
If function throw at least 1 exception, that neither argument count nor type mismatch, it should have `Exceptions` section, describing what exception means and when it thrown.
...
if `value` has type `Int`, prints it's value as string;
if `value` has type `Str`, prints it's value;
...
Throwed when failed to read from stdin.
...
Throwed when failed to open file.
...
Throwed when failed to read data from file.
...
Throwed when failed to write data to file.
...
Throwed when failed to parse string (found non-digit character or invalid minus position).
