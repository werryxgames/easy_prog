...
pub fn parse_expression_list(
    ...
    end_token: TokenType,
) -> Result<Vec<Rc<dyn AstNode>>, ParserError> {
    ...
    if first_expr.is_err() {
        unsafe {
            return Err(first_expr.unwrap_err_unchecked());
        }
    }
    ...
    while !tokens.is_empty() {
        ...
        if next_token.token_type == TokenType::Comma {
            ...
            return Err(ParserError::new(
                next_token.line,
                next_token.column,
                "Unexpected token type, expected comma (',')",
                ParserErrorCode::UnexpectedType,
            ));
        }
    }
...
pub fn parse_func_call(tokens: &mut Vector<Token>) -> Result<CallFuncNode, ParserError> {
    if tokens.is_empty() {
        return Err(ParserError::new(
            0,
            0,
            "Expected function call, but found end of file",
            ParserErrorCode::EmptyTokenList,
        ));
    }
    ...
    if tokens.len() == 1 {
        return Err(ParserError::new(
            0,
            0,
            "Expected left parentheses ('('), but found end of file",
            ParserErrorCode::ShortTokenList,
        ));
    }
    ...
    if token.token_type != TokenType::Lparen {
        return Err(ParserError::new(
            token.line,
            token.column,
            "Unexpected token type, expected left parentheses ('(')",
            ParserErrorCode::UnexpectedType,
        ));
    }
    ...
    if tokens.len() == 2 {
        return Err(ParserError::new(
            0,
            0,
            "Expected right parentheses (')'), but found end of file",
            ParserErrorCode::ShortTokenList,
        ));
    }
    ...
    let nodes_result: Result<Vec<Rc<dyn AstNode>>, ParserError> =
        parse_expression_list(tokens, TokenType::Rparen);
    ...
    Err(ParserError::new(
        last_token.line,
        last_token.column,
        "Unexpected token type, expected right parentheses (')')",
        ParserErrorCode::ShortTokenList,
    ))
}
...
pub fn parse_func_body(tokens: &mut Vector<Token>) -> Result<SequenceNode, ParserError> {
    if tokens.len() < 2 {
        return Err(ParserError::new(
            0,
            0,
            "Expected function body, but found end of file",
            ParserErrorCode::EmptyTokenList,
        ));
    }
    ...
    if token.token_type != TokenType::Lbrace {
        return Err(ParserError::new(
            token.line,
            token.column,
            "Unexpected token type, expected left brace ('{')",
            ParserErrorCode::UnexpectedType,
        ));
    }
    ...
    if last_token.token_type != TokenType::Rbrace {
        return Err(ParserError::new(
            token.line,
            token.column,
            "Unexpected token type, expected right brace ('}')",
            ParserErrorCode::UnexpectedType,
        ));
    }
...
pub fn parse_expression(tokens: &mut Vector<Token>) -> Result<Rc<dyn AstNode>, ParserError> {
    ...
    if token.token_type == TokenType::String {
        tokens.pop_front();
        let ast_box: Rc<dyn AstNode> = Rc::new(ConstStrNode::new(
            token.line,
            token.column,
            Str::new(&token.content),
        ));
        return Ok(ast_box);
    }
    ...
    if token.token_type == TokenType::Number {
        ...
        if result.is_err() {
            return Err(ParserError::new(
                ...
                ParserErrorCode::InvalidValue,
            ));
        }

        let ast_box: Rc<dyn AstNode> = Rc::new(ConstIntNode::new(
            token.line,
            token.column,
            Int::new(unsafe { result.unwrap_unchecked() }),
        ));
        return Ok(ast_box);
    }
    ...
    if token.token_type == TokenType::Identifier {
        if tokens.len() >= 2
            && unsafe { tokens.get(1).unwrap_unchecked() }.token_type == TokenType::Lparen
        {
        ...
        tokens.pop_front();
        let ast_box: Rc<dyn AstNode> =
            Rc::new(VariableNode::new(token.line, token.column, &token.content));
        return Ok(ast_box);
    }
    ...
    Err(ParserError::new(
        token.line,
        token.column,
        "Unexpected token type, expected expression",
        ParserErrorCode::UnexpectedType,
    ))
}
...
pub fn parse_program(tokens: &mut Vector<Token>) -> Result<SequenceNode, ParserError> {
    ...
    Ok(SequenceNode::new(1, 1, unsafe {
